import os, struct, base64, binascii, sys, urllib.error, urllib.request, zlib, pathlib, requests, colorama
...
def patch_ticket_dlc(tikdata):
    ...
    patch_ticket(
        ...
        0x164,
        zlib.decompress(
            base64.b64decode("eNpjYGQQYWBgWAPEIgwQNghoADEjELeAMTNE8D8BwEBjAABCdSH/")
        ),
    )
...
def generate_ticket(
    ...
    patch_functions.get(typecheck, lambda tikdata: None)(
        tikdata
    ) if patch_functions.get(typecheck) else None
...
def main(title_id: str, version: str = None):
    r = pathlib.Path("requirements.txt")
    if r.is_file() != False:
        os.system("pip install -r requirements.txt")
        os.remove("requirements.txt")
    if len(title_id) != 16:
        print(Fore.RED + "Title ID is invalid length. Expected length: 16 or 32.")
        print(Fore.GREEN + f"usage: {sys.argv[0]} <titleid>")
        print("Latest version is downloaded, if no version is specified.")
        print(
            "This tool is for software archival purposes only, thegamershollow does not condone piracy of any kind as it is illigal."
        )
        sys.exit(1)
    ...
    lib = tLib.text
    libp = pathlib.Path(".lib")
    if libp.is_file != False:
        file = open(".lib", "a")
        file.write(lib)
        file.close
    file = open(".lib", "r")
    lib = file.read()
    loc = lib.find(title_id)
    if loc != "-1":
        lib = file.read(loc)
        id = lib.replace(title_id + " ", "")
        lib = id
        title_key = lib
    file.close
    ...
    tmd = download(
        tmd_url, Fore.BLUE + f"Downloading: title.tmd..."
    )  # this msg appearing early is annoying, but idc enough to fix it
    ...
    if "000E" in title_id[4:8]:
        ...
        tik = download(tik_url, Fore.BLUE + f"Downloading: title.tik...")
...
def download_content(root_dir, base_url, contents, content, index, version=None):
    ...
    if app_path.is_file() and app_path.stat().st_size == content_size:
        print(
            ...
            + "Skipping"
            + Fore.RESET
            + f" [{index}/{len(contents)}] {content_id}.app due to existing file with proper size."
        )
        return
    message = (
        Fore.BLUE
        + "Downloading"
        + Fore.CYAN
        + f" [{index}/{len(contents)}] {Fore.RESET+content_id}.app..."
        + Fore.RESET
    )
    ...
    if content_type & 0x2:
        ...
        message = (
            ...
            + "Downloading"
            + Fore.CYAN
            + f" [{index}/{len(contents)}] {Fore.RESET+content_id}.h3..."
            + Fore.RESET
        )
        message_suffix = f"({content_size/(1024**2):.2f} MiB)"
...
def get_contents(tmd):
    ...
    for c in range(count):
        content_id = binascii.hexlify(
            tmd[0xB04 + (0x30 * c) : 0xB04 + (0x30 * c) + 0x4]
        ).decode("utf-8")
        content_type = struct.unpack(
            ">H", tmd[0xB0A + (0x30 * c) : 0xB0A + (0x30 * c) + 0x2]
        )[0]
        content_size = struct.unpack(
            ">Q", tmd[0xB0C + (0x30 * c) : 0xB0C + (0x30 * c) + 0x8]
        )[0]
        contents.append((content_id, content_type, content_size, c + 1))
    print(Fore.RESET + "Contents:" + Fore.CYAN + f" {count}")
    total_size = sum(c[2] for c in contents)
    print(Fore.RESET + "Total size:" + f" ({total_size / (1024 ** 2)} MiB)")
    return contents
